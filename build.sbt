...
lazy val commonSettings =
  Seq(
    ...
    version := "0.5",
    crossScalaVersions := Seq("2.13.5", "2.12.14", "2.11.12"),
    ...
    libraryDependencies ++= Seq("base", "controls", "fxml", "graphics", "media", "swing", "web").map(
      m => "org.openjfx" % s"javafx-$m" % "17-ea+12" % "provided" classifier osName),
...
lazy val demo = Project("scalafxml-demo-sfx8", file("demo"))
  ...
  .settings(
    ...
    libraryDependencies ++= Seq("base", "controls", "fxml", "graphics", "media", "swing", "web").map(
      m => "org.openjfx" % s"javafx-$m" % "17-ea+12" classifier osName)
  )
